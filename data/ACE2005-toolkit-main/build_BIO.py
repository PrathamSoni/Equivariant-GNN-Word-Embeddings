...
def get_entity_tag(raw_data):
    ...
    for i in range(len(raw_data)):
        ...
        test_strip = np.zeros(len(temp_sent['tokens']))
        ...
        for idx in index:
            ...
            l = length_list[idx]
            test_strip[temp_start:temp_start + l] = 1 + np.max(test_strip[temp_start:temp_start + l])
        n = max(np.max(test_strip), n)
    ...
    for sent in raw_data:
        ...
        for i in range(len(sent['tokens'])):
            partial.append(['O'] * int(n))
        entity_tag.append(partial)  # because of overlap,get a tag list for each token
    for i in range(len(raw_data)):
        temp_sent = raw_data[i]
        temp_entity_mentions = temp_sent['golden-entity-mentions']
        if len(temp_entity_mentions) == 0:  # skip if none
        ...
        for entity in temp_entity_mentions:
            ...
            length_list.append(entity['position'][1] - entity['position'][0] + 1)
        length_list = np.array(length_list)
        index = np.argsort(length_list)  # get the length idx from long to short
        ...
        for idx in index:
            ...
            to_fill = int(np.max(test_strip[temp_start: temp_start + l]) - 1)

            for k in range(l):
                if k == 0:
                    entity_tag[i][temp_start + k][to_fill] = ('B-' + type_list[idx])
                else:
                    entity_tag[i][temp_start + k][to_fill] = ('I-' + type_list[idx])
